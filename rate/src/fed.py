...
class Federation:
    ...
    def split_model(self, user_idx):
        if cfg['model_name'] == 'conv':
            ...
            print("output_weight_name", output_weight_name, "output_bias_name", output_bias_name)
            for k, v in self.global_parameters.items():
                #print("k, v.dim()", k, v.shape)
...
#    return new_solutions
    ...
    def combine2(self, local_parameters, param_idx, user_idx, hs, Deltas):
        demominator = np.sum(np.asarray(hs))
        scaled_deltas = []
        ...
        #print("scaled_deltas", scaled_deltas)
        #print("self.global_parameters", self.global_parameters)
        #scaled_deltas = local_parameters
                        ...
                        if parameter_type == 'weight':
                            if v.dim() > 1:
                                if k == output_weight_name:
                                    ...
                                    tmp_v[torch.meshgrid(param_idx[m][k])] += scaled_deltas[m][k][label_split]
                                    count[k][torch.meshgrid(param_idx[m][k])] += 1
                                else:
                                    tmp_v[torch.meshgrid(param_idx[m][k])] += scaled_deltas[m][k]
                                    count[k][torch.meshgrid(param_idx[m][k])] += 1
                            else:
                                tmp_v[param_idx[m][k]] += scaled_deltas[m][k]
                                count[k][param_idx[m][k]] += 1
                        else:
                            if k == output_bias_name:
                                label_split = self.label_split[user_idx[m]]
                                param_idx[m][k] = param_idx[m][k][label_split]
                                tmp_v[param_idx[m][k]] += scaled_deltas[m][k][label_split]
                                count[k][param_idx[m][k]] += 1
                            else:
                                tmp_v[param_idx[m][k]] += scaled_deltas[m][k]
                                count[k][param_idx[m][k]] += 1
                    else:
                        tmp_v += scaled_deltas[m][k]
                        count[k] += 1
