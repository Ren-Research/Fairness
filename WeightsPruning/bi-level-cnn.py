...
#            print([len(dict_users[k]) for k in dict_users.keys()])
	...
	if args.test_loss:
		dict_users_test, _ = partition_data(dataset_test, args.num_users, "noniid-#label10")
	...
	dict_users_test, _ = partition_data(dataset_test, args.num_users, "noniid-labeldir")
...
#                            acc_test, loss_test = test_img(model, dataset_test, args)
					...
					if args.q > 0:
						...
						hs.append((args.q * np.float_power(loss+1e-10, (args.q-1)) * norm_grad(grads) + (1.0/args.lr) * np.float_power(loss+1e-10, args.q)) / 1)
