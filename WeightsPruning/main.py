...
from models.Fed import FedAvg, FedAvg2, aggregate
...
def get_half_paras(w_glob):
    w_l = copy.deepcopy(w_glob)
    print(w_glob.keys(), [w_glob[key].shape for key in w_glob.keys()])
    w_l['layer_input.weight'] = w_l['layer_input.weight'][:100, :]
    w_l['layer_input.bias'] = w_l['layer_input.bias'][:100]
    w_l['layer_hidden.weight'] = w_l['layer_hidden.weight'][:, :100]
...
if __name__ == '__main__':
    ...
    if args.model == 'cnn' and args.dataset == 'cifar':
        ...
        print(net_glob)
        net = MLP(dim_in=len_in, dim_hidden=100, dim_out=args.num_classes).to(args.device)
        print(net)
...
7 net_53                                     X               X
    ...
    w_half = get_half_paras(w_glob)
    net.load_state_dict(w_half)
...
##########################TEMP USE FOR STRATING LOSS
        ...
        for iter in range(args.epochs):
            ...
            for id, idx in enumerate(idxs_users):
                ...
                if typep == 1:
                    type_array.append(1)
                    w, loss, m = local.train(net=copy.deepcopy(net_glob).to(args.device))
                    m.eval()
                    acc_test, loss_test = test_img(m, dataset_test, args)
                    print(id, acc_test, loss_test, loss)
                elif typep == 2:
                    type_array.append(2)
                    #w, loss = local.train(net=copy.deepcopy(net_2).to(args.device))
                    #w = get_sub_paras(w, local_w_masks[1], local_b_masks[1])
                    w, loss, m = local.train(net=copy.deepcopy(net).to(args.device))
                    m.eval()
                    acc_test, loss_test = test_img(m, dataset_test, args)
                    w = get_half_paras(w)
                    print(id, acc_test, loss_test, loss)
